...
export const adminServiceRoutes = async (fastify) => {
    // GET /api/admin/services
    fastify.get('/services', { preHandler: [fastify.authenticate] }, async () => {
    ...
    fastify.put('/services/reorder', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    fastify.patch('/services/:id/publish', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const user = request.adminUser;
        ...
        const newStatus = request.body?.isActive !== undefined
    ...
    fastify.put('/services/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const user = request.adminUser;
        const { id } = request.params;
        const body = request.body;
        const existing = db.services.findById(id);
        ...
        if (body.slug !== undefined)
            updates.slug = body.slug ? body.slug.trim() : null;
        if (body.icon !== undefined)
            updates.icon = (body.icon || 'Music').trim();
        ...
        if (body.ctaText !== undefined)
            updates.ctaText = body.ctaText;
        if (body.pricingRange !== undefined)
            updates.pricingRange = body.pricingRange;
    ...
    fastify.delete('/services/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const user = request.adminUser;
        const { id } = request.params;
        const existing = db.services.findById(id);
        if (!existing) {
