...
import type { Artist } from '../../db/types.js';

export const adminArtistRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.put<{
    Params: { id: string };
    Body: Partial<Artist> & { socials?: Array<{ platform: string; url: string }>; published?: boolean };
  }>(
    '/artists/:id',
    { preHandler: [fastify.authenticate] },
    async (request, reply) => {
      ...
      if (updates.genres && typeof updates.genres === 'string') {
        updates.genres = (updates.genres as string).split(',').map((g) => g.trim()).filter(Boolean);
      }
