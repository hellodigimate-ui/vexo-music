...
export class LocalStorageProvider implements StorageProvider {
   ...
   */
  public detectCategory(mimeType: string): StorageCategory {
   ...
   */
  public async delete(pathOrFilename: string): Promise<boolean> {
    try {
   ...
   */
  public getUrl(pathOrFilename: string): string {
    const sanitized = pathOrFilename.replace(/^[/\\]+/, '').replace(/\\/g, '/');
