...
export const EventTable: React.FC<EventTableProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-[#0e0e13] border border-slate-200 dark:border-zinc-800/80 rounded-2xl overflow-hidden shadow-sm dark:shadow-2xl">
            ...
            {events.map((event) => {
              ...
              return (
                <tr key={event.id} className="hover:bg-slate-50/80 dark:hover:bg-zinc-900/40 transition-colors group">
                  ...
                  <td className="py-4 px-6">
                    <div className="flex flex-col">
                      <span className="font-semibold text-slate-900 dark:text-zinc-200">{event.venue}</span>
                  ...
                  <td className="py-4 px-6">
                    <div className="flex flex-wrap items-center gap-1.5 max-w-xs">
                      {artistList.map((artName, i) => (
                        <span
                          key={i}
                          className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-mono bg-red-50 text-red-700 border border-red-200 dark:bg-vexo-red/10 dark:text-zinc-200 dark:border-vexo-red/20"
                        >
                  ...
                  <td className="py-4 px-6">
                    ...
                    >
                      <Star className={`w-3 h-3 ${isFeatured ? 'fill-amber-500 dark:fill-amber-400 text-amber-500 dark:text-amber-400' : ''}`} />
                  ...
                  <td className="py-4 px-6 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <NavLink
                        to={`/admin/events/${event.id}/edit`}
                        title="Edit Event"
                        className="p-2 rounded-xl bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-zinc-800 transition-colors"
                      >
