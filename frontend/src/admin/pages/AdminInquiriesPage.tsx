  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-white tracking-wide uppercase flex items-center gap-2.5">
            <Mail className="w-5 h-5 text-vexo-red-bright" />
        ...
        <button
          ...
          className="self-start sm:self-auto px-3 py-1.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-xs text-zinc-300 hover:text-white flex items-center gap-1.5 transition-colors cursor-pointer"
          title="Refresh List"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
        <div className="p-4 rounded-2xl bg-[#0e0e13] border border-zinc-800/80 space-y-1">
        ...
        <div className="p-4 rounded-2xl bg-[#0e0e13] border border-red-900/40 bg-gradient-to-br from-red-950/20 to-transparent space-y-1 relative overflow-hidden">
          ...
          <p className="text-2xl font-black font-mono text-vexo-red-bright">{metrics.newCount}</p>
        </div>

        <div className="p-4 rounded-2xl bg-[#0e0e13] border border-sky-900/30 space-y-1">
          <div className="flex items-center justify-between text-sky-400">
            <span className="text-[10px] font-mono uppercase tracking-wider font-bold">CONTACTED</span>
            <PhoneCall className="w-4 h-4 text-sky-400" />
          </div>
          <p className="text-2xl font-black font-mono text-sky-400">{metrics.contactedCount}</p>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-[#0e0e13] border border-zinc-800/80 space-y-1">
          ...
          <p className="text-2xl font-black font-mono text-emerald-400">{metrics.closedCount}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        {/* Status Filter Tabs */}
        <div className="flex flex-wrap items-center gap-1.5 p-1 bg-[#0e0e13] border border-zinc-800 rounded-xl">
          {statusTabs.map((tab) => {
      ...
      <div className="bg-[#0e0e13] border border-zinc-800/80 rounded-2xl overflow-hidden shadow-2xl">
        {isLoading ? (
          <div className="py-20 text-center text-xs font-mono text-zinc-500 flex flex-col items-center justify-center gap-3">
          ...
          <div className="py-20 text-center space-y-2">
            ...
            <p className="text-sm font-semibold text-zinc-300">No contact requests found</p>
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-2">
                          ...
                          {isNew && (
                            <span className="inline-flex items-center px-1.5 py-0.2 rounded text-[9px] font-mono font-bold bg-red-950 border border-red-800 text-red-300">
                              NEW
                            </span>
                          )}
                        </div>
                        <p className="font-bold text-white text-xs mt-0.5">{inq.name}</p>
                        {inq.company && (
                      ...
                      <td className="py-4 px-6">
                        <a
                          ...
                          className="font-mono text-zinc-200 hover:text-vexo-red transition-colors block"
                        >
                          {inq.email}
                        </a>
                        {inq.phone && (
                      ...
                      {/* Service */}
                      <td className="py-4 px-6">
                        <span className="inline-block px-2.5 py-1 rounded-lg bg-zinc-900 border border-zinc-800 text-[11px] font-mono font-medium text-zinc-300">
                          {inq.service}
                        </span>
                      </td>

                      {/* Date */}
                      <td className="py-4 px-6 font-mono text-[11px] text-zinc-400 whitespace-nowrap">
                        ...
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${
                              isNew
                                ? 'bg-vexo-red-bright animate-pulse'
                                : isContacted
                                ? 'bg-sky-400'
                                : 'bg-emerald-400'
                            }`}
                        ...
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          {inq.status === 'NEW' && (
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-lg bg-sky-950/60 hover:bg-sky-900/80 border border-sky-800/60 text-sky-300 text-[11px] font-mono font-semibold transition-colors cursor-pointer flex items-center gap-1"
                          ...
                          {inq.status !== 'CLOSED' && (
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-lg bg-emerald-950/60 hover:bg-emerald-900/80 border border-emerald-800/60 text-emerald-300 text-[11px] font-mono font-semibold transition-colors cursor-pointer flex items-center gap-1"
                          ...
                          <button
                            type="button"
                            onClick={() => handleOpenDetail(inq)}
                            className="px-2.5 py-1.5 rounded-lg bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-300 hover:text-white text-[11px] font-semibold transition-colors cursor-pointer flex items-center gap-1"
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg bg-red-950/30 hover:bg-red-900/60 border border-red-900/40 text-red-400 hover:text-red-200 transition-colors cursor-pointer"
        ...
        >
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <span className="text-xs font-mono text-zinc-400 uppercase">REQUESTED SERVICE:</span>
                <span className="px-3 py-1 rounded-lg bg-red-950 border border-red-800 text-xs font-mono font-bold text-red-300">
              ...
              <div>
                ...
                <div className="p-4 rounded-xl bg-black/80 border border-zinc-800/80 text-xs text-zinc-200 leading-relaxed whitespace-pre-wrap font-sans">
                  ...
                  return (
                    <button
                      ...
                      className={`py-2.5 px-3 rounded-xl text-xs font-mono font-bold transition-all cursor-pointer flex items-center justify-center gap-2 ${
                        isCurrent
                          ? st === 'NEW'
                            ? 'bg-red-600 text-white shadow-[0_0_20px_rgba(220,38,38,0.5)] ring-2 ring-red-400'
                            : st === 'CONTACTED'
                            ? 'bg-sky-600 text-white shadow-[0_0_20px_rgba(2,132,199,0.5)] ring-2 ring-sky-400'
                            : 'bg-emerald-600 text-white shadow-[0_0_20px_rgba(5,150,105,0.5)] ring-2 ring-emerald-400'
            ...
            <div className="space-y-2 pt-2 border-t border-zinc-800">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-mono text-zinc-500">Visible to admins only</span>
              </div>
              <textarea
                rows={3}
                value={adminNotes}
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-zinc-900 border border-zinc-800 text-xs text-white placeholder-zinc-600 focus:border-vexo-red focus:outline-none resize-none leading-relaxed"
              />
              <div className="flex justify-end">
                <button
                  ...
                  disabled={isUpdating}
                  className="px-4 py-1.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-xs font-semibold text-white transition-colors cursor-pointer"
                >
                  Save Notes
                </button>
              </div>
