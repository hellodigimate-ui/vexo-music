...
export const FeaturedSingleBanner: React.FC = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([homepageApi.getHomepage(), videosApi.getVideos()]).then(([homeRes, vidRes]) => {
      ...
      if (!featured) {
        featured = vids.find((v: any) => v.featured) || vids[0];
      }

      if (featured) {
        setVideoDetails({
          title: featured.title,
          artists: featured.artist || 'VEXO Artists',
          label: 'Vexo Entertainment Pvt. Ltd.',
          youtubeId: featured.youtubeId || 'HcEcM5AtEZ8',
          youtubeUrl: `https://youtu.be/${featured.youtubeId}`,
          thumbnailUrl: featured.thumbnailUrl || `https://img.youtube.com/vi/${featured.youtubeId}/maxresdefault.jpg`,
          fallbackThumbnail: `https://img.youtube.com/vi/${featured.youtubeId}/hqdefault.jpg`,
          likes: String(featured.likes || '62'),
          views: String(featured.views || '835'),
          releaseDate: featured.publishedAt || '4 Aug 2026',
          hashtags: featured.tags && featured.tags.length > 0 ? featured.tags : ['#VEXO', '#OfficialRelease', '#MusicVideo'],
